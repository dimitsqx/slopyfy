...
function buildProductCardHtml(product) {
    const colorChips = product.colors
        .map((color) => `<span style="display:inline-block;padding:4px 8px;border-radius:999px;background:#f2f4f5;color:#334155;font-size:12px;">${escapeHtml(color)}</span>`)
        .join('');
...
function buildProductCardResource(product) {
    return {
        type: 'resource',
        resource: {
...
function buildProductCardApp(product) {
    return {
        name: 'product-card',
        title: product.name,
...
function buildProductDetailsHtml(product) {
    const colorItems = product.colors
        .map((color) => `<li style="margin:0;padding:0;color:#374151;font-size:14px;">${escapeHtml(color)}</li>`)
        .join('');
    const sizeItems = product.sizes
        .map((size) => `<li style="margin:0;padding:0;color:#374151;font-size:14px;">${escapeHtml(size)}</li>`)
        .join('');
    return `<!DOCTYPE html>
<html lang="en">
  <head>
...
function buildProductDetailsResource(product) {
    return {
        type: 'resource',
        resource: {
...
function buildProductDetailsApp(product) {
    return {
...
export function createShopServer() {
    ...
    server.registerTool('sample_product_card', {
