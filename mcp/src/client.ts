...
async function main() {
  ...
  const products = (listResult.structuredContent as { products: Array<{ id: string; name: string; priceUsd: number }> })
    .products;
  ...
  const selectedId =
    !Number.isNaN(byIndex) && byIndex > 0 && byIndex <= products.length
